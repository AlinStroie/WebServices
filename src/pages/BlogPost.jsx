...
function renderBlock(block) {
    ...
    return (
      <ul key={block.items.join("-")} className="mt-6 space-y-3">
        {block.items.map((item) => (
          <li key={item} className="flex gap-3 text-lg leading-8 text-white/60">
  ...
  return (
    <p key={block.text} className="mt-6 text-lg leading-9 text-white/62">
      {block.text}
    </p>
  );
}
...
function RelatedPostCard({ post }) {
  return (
    <Link
      to={`/blog/${post.slug}`}
      className="group block rounded-[1.5rem] border border-white/10 bg-white/[0.045] p-4 shadow-[0_18px_70px_rgba(0,0,0,0.35)] backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:bg-white/[0.065]"
    >
      <div className="mb-4 flex items-center justify-between gap-4">
        <span className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-xs text-white/42">
          {post.category}
        </span>
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#050505] pb-28 text-white">
      ...
      <div className="pointer-events-none fixed inset-0 z-0">
        ...
        <div className="absolute left-[-14rem] top-24 h-[36rem] w-[36rem] rounded-full bg-white/[0.035] blur-[130px]" />
        <div className="absolute right-[-14rem] top-72 h-[34rem] w-[34rem] rounded-full bg-[#b7aa8f]/[0.045] blur-[150px]" />
          ...
          <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_22rem] lg:gap-14">
            <article className="min-w-0">
              <section className="pb-12 pt-8">
                <Link
                  to="/blog"
                  className="mb-8 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-sm font-semibold text-white/55 transition hover:bg-white hover:text-black"
                >
                  <ArrowLeft size={16} />
                  Înapoi la articole
                </Link>

                <div className="mb-6 flex flex-wrap items-center gap-3 text-sm text-white/42">
              ...
              <div className="relative mb-12 h-80 overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.045] text-white shadow-[0_24px_90px_rgba(0,0,0,0.42)] backdrop-blur-xl">
              ...
              <section className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-6 shadow-[0_24px_90px_rgba(0,0,0,0.35)] backdrop-blur-xl md:p-10">
                <div className="mx-auto max-w-3xl">{post.content.map(renderBlock)}</div>
              </section>

              <section className="my-12 overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.05] p-7 text-white shadow-[0_24px_90px_rgba(0,0,0,0.38)] backdrop-blur-xl">
            ...
            <aside className="lg:pt-32">
              <div className="sticky top-28 space-y-5">
                <div className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-5 shadow-[0_18px_70px_rgba(0,0,0,0.35)] backdrop-blur-xl">
                  <p className="text-sm uppercase tracking-[0.3em] text-white/35">
                    Continuă lectura
                  </p>
                ...
                <div className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-5 shadow-[0_18px_70px_rgba(0,0,0,0.35)] backdrop-blur-xl">
                  ...
                  <div className="mt-4 grid gap-2">
                    <Link
                      to="/blog"
                      className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-3 text-center text-sm font-semibold text-white/65 transition hover:bg-white hover:text-black"
                    >
                      Toate articolele
                    </Link>
